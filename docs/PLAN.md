...
- 公開: GitHub Pages（配信元を `fable_ver` ブランチに設定。main は変更しない）
...
| 6 | 仕上げと公開: Web Audio 効果音、タッチUI調整、README、GitHub Pages 配信元切替＋動作確認 | `feature/fable-polish` | ⬜ 未着手 |
...
```
gh api -X PUT repos/unkei/ai-rtype/pages \
  -f "source[branch]=fable_ver" -f "source[path]=/"
```

main ブランチ自体には commit / push / checkout しない。
